...
- **状态**：`[x]` 已修复（2026-09-18，含二审返工）：`is_newer` 改用带预发布语义的可比较键（数字三元组 + 正式版标志 + 后缀标识段），正式版大于同数字段一切预发布；后缀点分标识按 SemVer 区分大小写并逐段比较（数字标识编码为可跨类型安全比较的三元组，按数值比较、低于字母数字标识，不抛 TypeError；超 64 位纯数字标识按字母数字处理，避开 CPython int 转换位数上限），`+build` 元数据不影响结果。非法输入语义显式化：`_release_key` 解析失败返回 None，`is_newer` 对非法远端一律 False、非法本地沿用旧 `0.0.0` 语义；`check_for_update` 先 `fullmatch` 校验远端 tag，垃圾 tag 直接抛错而不是折叠成 `0.0.0` 走更新流。`parse_version` 与 `canonical_version` 的数字剥离契约保持不变（后者是 v0.1.28 的路径穿越防线）。生产远端是 GitHub `/releases/latest`（只含正式版），故缺口仅影响本地预发布构建升级，已由该修复覆盖；未引入 `packaging` 依赖。
