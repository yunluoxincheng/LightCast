...
- **状态**：`[x]` 已修复（2026-08-10）：混合 HLS 启动阶段只同步预热分片 0，确保 mpv 首片请求命中；随后通过 `_schedule_warm(0)` 把分片 1–3 注册到代理的 `BackgroundTasks` 后台预取，setup 不再等待这些任务。代理停止时沿用既有 `cancel_all()`，会先取消并等待所有预热任务，再关闭 session。测试验证初始化返回前只执行首片、后续任务异步启动且停止时完整取消。
