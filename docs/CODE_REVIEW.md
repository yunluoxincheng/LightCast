...
- **状态**：`[x]` 已修复（2026-09-18）：checkout / setup-python / upload-artifact 的全部 7 处 `uses` 引用从浮动 `@v5/@v6/@v4` 钉死为对应 release tag 的完整 commit SHA（v5.1.0 / v6.3.0 / v4.6.2），workflow 头部注明「升级须有意更换 SHA 并同步回归测试」；新增回归测试强制所有 `uses` 引用保持 40 位 SHA 并锁定 action 集合。审查建议中的 release job 环境保护 / 手动确认与发布 Goal「合并 master 即发布批准」的既定决策冲突，维持不加。
