...
- **状态**：`[x]` 已修复（2026-09-18）：`is_newer` 改用带预发布语义的可比较键（数字三元组 + 正式版标志 + 后缀标识段），正式版大于同数字段一切预发布；后缀点分标识按 SemVer 逐段比较，数字标识编码为可跨类型安全比较的三元组（按数值比较、低于字母数字标识，不抛 TypeError），`+build` 元数据不影响结果。`parse_version` 与 `canonical_version` 的数字剥离契约保持不变（后者是 v0.1.28 的路径穿越防线）。生产远端是 GitHub `/releases/latest`（只含正式版），故缺口仅影响本地预发布构建升级，已由该修复覆盖；未引入 `packaging` 依赖。
