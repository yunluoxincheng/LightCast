...
- **状态**：`[x]` 已修复（2026-08-29，含两轮 PR 审查返工）：建立「控制点授权」模型，覆盖全部改变播放状态的 action，且授权是任何状态变更的第一道门槛。server 层补丁包装库模块级 `action_handler`，在 SOAP 控制请求入口用 `request.remote` 把真实控制点 IP 登记进 ContextVar（`_control_context.py`，按请求任务隔离）；Bridge 据此授权：首次来自新控制点的 `SetAVTransportURI` 在通过 scheme/SSRF 校验后弹窗确认（自动唤起主窗口，30 秒未确认自动拒绝），确认通过之前不向 service 写任何 evented 状态（候选 URI 不会在弹窗期间暴露给 GENA 订阅者），同意后授权该控制点本次运行内的 `SetURI / Play / Pause / Stop / Seek / SetVolume / SetMute`；未授权或拿不到控制点 IP（上下文缺失、门控未装配）的任何状态变更请求直接抛 `UpnpActionError` 走 SOAP fault（含 SetURI——控制点不会误以为投屏已被接受），fail-closed。`RenderingControlService` 的 SetVolume/SetMute 调整为先经 bridge 授权再写 evented 状态。授权身份是真实 peer IP，媒体 URL host 不充当设备身份——不同设备投同一 CDN / NAS 域名仍需分别确认，杜绝授权串用；`require_cast_confirm` 关闭时进入用户显式选择的无鉴权模式。「仅通过默认网卡投屏」开关（默认关闭）：HTTP/SOAP 只绑 `get_local_ip()`，SSDP membership / sender / M-SEARCH 选卡与 fallback 收窄到 `filter_interfaces` 白名单，白名单子网外的 M-SEARCH 直接忽略（重启投屏服务后生效）。README 明示公共 Wi-Fi 风险。service 层测试还暴露并修复了一个存量 bug：`Mute` 状态变量是 boolean 类型，旧代码写 `"1"/"0"` 字符串被库 schema 拒绝（`UpnpValueError`）且被 `_write_state_variable` 的 try/except 吞掉，静音状态从未真正同步过，现统一写真实 bool。新增测试覆盖：跨控制点授权串用（同域名不同设备）、service 层未授权 SetVolume/SetMute/SetURI 状态零副作用、确认期间候选 URI 不发布、未授权/未知控制点与门控未注入 fail-closed、SSRF 拦截不弹窗、请求上下文隔离、补丁登记上下文、M-SEARCH 白名单边界、绑定地址随配置切换与下载目标目录包含校验。
