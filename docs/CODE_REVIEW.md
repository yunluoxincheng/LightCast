...
- **状态**：`[x]` 核心绕过已修复（2026-08-09）：仅允许 http/https；所有上游请求关闭自动重定向并逐跳校验；`SSRFSafeConnector` 在 aiohttp 连接解析阶段过滤实际候选 IP，使校验结果与建连使用同一批地址；安全拒绝统一抛 `UrlBlockedError`，mpv 只接收本地代理 URL，不再存在原链 fallback。设置页「允许内网投屏源」默认开以兼容 DLNA 私网媒体，因此主机可达的 RFC1918 / ULA 私网地址仍可作为媒体源；关闭该选项可进入拒绝私网的严格模式。无论此选项是否开启，回环、link-local、云元数据、未指定、多播、CGNAT 和保留地址始终拒绝。
...
- **影响**：历史实现允许局域网攻击者借受害者机器读取本机 / 内网服务或探测内网。当前默认策略已阻断本机、link-local、云元数据及各类绕过，但为保证局域网投屏兼容性，仍接受“可访问任意 RFC1918 / ULA 私网媒体源”的残余风险，包括路由器、VPN、虚拟网卡和第二张网卡可达的地址。
- **建议**：
  - 已完成：投屏 URL 和 m3u8 内所有 URL 仅允许 `http` / `https`，逐跳校验重定向，并在实际建连解析阶段过滤始终禁止的地址。
  - 已完成：代理失败或安全拒绝时终止播放，不再把未经保护的原始 URL 交给 mpv。
  - 当前产品边界：「允许内网投屏源」默认开启；用户可关闭它，拒绝 RFC1918 / ULA 私网媒体源。
  - 后续安全增强：把 SOAP 请求远端 IP 传入播放链路，默认只允许投屏发起设备自身提供私网媒体；同一私网 host 的 HLS segment/key/map 可继续访问，跳转到其他私网 host 则拒绝。需要访问任意私网源时再由用户显式放宽。

### 🔴 C3. 测试目录为空，核心模块 0% 覆盖；CI 无任何质量门禁

- **文件**：`tests/`；`.github/workflows/release.yml`
- **状态**：`[x]` 安全关键路径已建立正式回归门禁（2026-08-21）：158 个 pytest 用例覆盖 SSRF 重定向/解析/fallback、私网开关、AES key 边界、Pillow 像素限制、更新 SHA-256、流式上游超时与连接池限制、HLS 启动/后台预热、缓存 single-flight 与代理路由索引校验、后台任务退出清理、SSDP 线程生命周期、mpv shutdown 屏障、工作线程到 Qt 主线程的投递、DLNA 时间/错误状态/并发 SetURI/代理资源事务、窗口几何迁移与多屏实际拖动区可见性、真实 QWidget Show 及 QApplication/qasync 退出时序；PR 与发布构建均先运行 Windows test job。全项目覆盖率、ruff/mypy/bandit 仍属后续工程化工作。
...
- **状态**：`[x]` 已修复（2026-08-10）：listener 用启动事件把 socket 初始化成功/失败同步给 `DlnaServer.async_start()`，服务只有在 SSDP 明确就绪后才标记运行；多播 alive/byebye 与 `_cleanup()` 通过同一资源锁串行操作 sender/socket，alive 取得锁后会复查运行状态，杜绝 byebye 后再次广播；`stop()` 发送 byebye、清运行标志、唤醒 `recvfrom` 后执行有界 `join`，线程超时未退出时抛出异常并保留 listener 引用供统一清理链路重试。同步的启动等待和停止 join 均通过 `asyncio.to_thread()` 执行，不阻塞 qasync/Qt 事件循环。新增测试覆盖正常退出、alive/byebye 竞态、事件循环可调度、部分启动资源清理、启动失败传播和停止超时引用保留。
- **问题**：
...
- **状态**：`[x]` 已修复（2026-08-10）：新增 `RLock` 统一保护播放器缓存字段的跨线程读写；所有 mpv property/event callback 在锁内检查 `_shutting_down`，`shutdown()` 先置关闭标志、切断 `_mpv` 与 attached 可见性、使已排队信号世代失效，再在锁外调用会 join 事件线程的 `python-mpv terminate()`。mpv 工作线程不再直接发射公开信号，而是经显式 `QueuedConnection` 投递给 PlayerSignals 的主线程 `@Slot` dispatcher，再由 dispatcher 通知 UI、RendererBridge 及普通 Python function/lambda；外部接收者不会在播放器锁内或 MPVEventHandlerThread 中执行。测试同时覆盖 terminate 期间/返回后的晚到回调，以及真实 Python worker thread 发射后普通函数接收者在 `QCoreApplication` 线程执行。
...
- **状态**：`[x]` 已修复（2026-08-29，含三轮 PR 审查返工）：建立「控制点授权」模型，覆盖全部改变播放状态的 action，且授权是任何状态变更的第一道门槛。server 层补丁包装库模块级 `action_handler`，在 SOAP 控制请求入口用 `request.remote` 把真实控制点 IP 登记进 ContextVar（`_control_context.py`，按请求任务隔离）；Bridge 据此授权：首次来自新控制点的 `SetAVTransportURI` 在通过 scheme/SSRF 校验后弹窗确认（自动唤起主窗口，30 秒未确认自动拒绝），确认通过之前不向 service 写任何 evented 状态（候选 URI 不会在弹窗期间暴露给 GENA 订阅者），同意后授权该控制点本次运行内的 `SetURI / Play / Pause / Stop / Seek / SetVolume / SetMute / SetNextAVTransportURI`；未授权或拿不到控制点 IP（上下文缺失、门控未装配）的任何状态变更请求直接抛 `UpnpActionError` 走 SOAP fault（含 SetURI——控制点不会误以为投屏已被接受），fail-closed。`ensure_authorized_controller(action)` 是 Bridge 暴露给 service 的公开授权接口，所有写 action 在状态变更前第一行调用；service 在 `RendererBridge` 未装配时也一律 fail-closed（不再保留"直接写 service 状态"的降级路径），`DlnaServer.async_start` 在缺 service 时直接启动失败，杜绝「安全组件未装配但 SOAP 写接口仍开放」的半失效状态。`RenderingControlService` 的 SetVolume/SetMute 调整为先经 bridge 授权再写 evented 状态。授权身份是真实 peer IP，媒体 URL host 不充当设备身份——不同设备投同一 CDN / NAS 域名仍需分别确认，杜绝授权串用；`require_cast_confirm` 关闭时进入用户显式选择的无鉴权模式。「仅通过默认网卡投屏」开关（默认关闭）：HTTP/SOAP 只绑 `get_local_ip()`，SSDP membership / sender / M-SEARCH 选卡与 fallback 收窄到 `filter_interfaces` 白名单，白名单子网外的 M-SEARCH 直接忽略（重启投屏服务后生效）。README 明示公共 Wi-Fi 风险。service 层测试还暴露并修复了一个存量 bug：`Mute` 状态变量是 boolean 类型，旧代码写 `"1"/"0"` 字符串被库 schema 拒绝（`UpnpValueError`）且被 `_write_state_variable` 的 try/except 吞掉，静音状态从未真正同步过，现统一写真实 bool。新增测试覆盖：跨控制点授权串用（同域名不同设备）、service 层未授权 SetVolume/SetMute/SetURI/SetNextURI 状态零副作用、确认期间候选 URI 不发布、未授权/未知控制点与门控未注入 fail-closed、bridge 缺失 fail-closed、SSRF 拦截不弹窗、请求上下文隔离、补丁登记上下文、M-SEARCH 白名单边界、绑定地址随配置切换与下载目标目录包含校验。
...
- **状态**：`[x]` 已修复（2026-08-11）：`00:00:00` 现在按合法的 0 秒解析，仅空字符串和 `NOT_IMPLEMENTED` 保持未实现语义，并统一处理首尾空白。
...
- **文件**：`ydlna/player/hls_rewriter.py:313-420, 642-863, 931-948`
- **状态**：`[x]` 已修复（2026-08-11）：代理基类新增按资源 key 管理的 single-flight 注册表，AES key、混合 HLS 分片、图片转码和 DirectProxy 整段缓冲在缓存 miss 时只创建一个受管生产任务，其余并发请求共享同一结果。混合分片的后台预热与播放器即时请求也先使用同一个 flight，消除 PR #7 后仍存在的重复上游抓取窗口；如果 `retries=0` 且无播放器请求头的预热失败，已加入的前台请求会再以自己的 headers 和 `retries=1` 串行创建或加入 foreground flight，保留原有播放恢复能力而不产生并发抓取。单个请求取消只结束自身等待，不会误杀其他使用者共享的任务；代理停止仍通过 `BackgroundTasks.cancel_all()` 统一取消并等待生产任务。成功、失败或取消都会清除 flight，失败不会污染缓存且后续请求可重试。新增测试覆盖密钥取消隔离、失败共享/重试、预热与即时请求合流、预热失败后的前台接管、图片转换和直链缓冲去重。
- **问题**：判空 → 抓取 → 写缓存无锁，同分片 / key 被并发重复抓取。
- **建议**：用 `asyncio.Lock` 或 single-flight 模式（每个 key 一个 `Future`，后续请求 await 同一 Future）。

### M5. 全局快捷键未按页面可见性门控

- **文件**：`ydlna/player/player_interface.py:52-94`
- **状态**：`[x]` 已修复（2026-08-11）：应用级事件过滤器在保留文本输入框和下拉框优先放行后，统一检查播放器页面可见性；页面隐藏时不再查找、调用或消费任何媒体快捷键，页面可见时原有空格、方向键、音量和全屏快捷键行为保持不变。新增测试直接覆盖隐藏页面不触发播放控制和可见页面正常消费快捷键。
...
- **状态**：`[x]` 已修复（2026-09-18，含两轮返工）：`is_newer` 改用带预发布语义的可比较键（数字三元组 + 正式版标志 + 后缀标识段），正式版大于同数字段一切预发布。后缀用独立严格 SemVer 2.0 正则 fullmatch 解析（与 `canonical_version` 的宽松剥离职责分离，版本携带任何非法字符即无法解析）：预发布标识区分大小写、逐段比较；数字标识按「有效位数 + 去前导零数字串」编码（不转 `int`，任意长度真正按数值比较、无位数上限风险），且始终低于字母数字标识；`+build` 元数据不参与比较。非法输入语义显式化：解析失败返回 None，`is_newer` 对非法远端一律 False、非法本地沿用旧 `0.0.0` 语义；`check_for_update` 先按严格 SemVer 校验远端 tag，垃圾 tag 直接抛错而不是折叠成 `0.0.0` 走更新流。`parse_version` 与 `canonical_version` 的数字剥离契约保持不变（后者是 v0.1.28 的路径穿越防线）。生产远端是 GitHub `/releases/latest`（只含正式版），故缺口仅影响本地预发布构建升级，已由该修复覆盖；未引入 `packaging` 依赖。
...
- **状态**：`[x]` 已修复（2026-08-21）：启动恢复前读取所有 `QScreen.availableGeometry()`，并按当前 `MSFluentTitleBar` 中未隐藏的真实 `TitleBarButton.width()` 动态扣除右侧最小化、最大化和关闭按钮区；剩余顶部 `48px` 实际拖动区域必须与任一屏幕工作区存在至少 `80×24` 的可操作交集。完全落在已拔除副屏、仅剩极小边缘、只有窗口底部内容或右侧控制按钮可见时清除失效 geometry，回退到系统可见位置和默认 `1200×800`。合法的多屏负坐标和副屏自定义位置仍正常恢复。新增测试覆盖主屏/副屏、拔屏、负坐标及左右/上下对称边界。
...
- **状态**：`[x]` 已修复（2026-08-10）：`shutdown()` 会断开 player 的状态/音量/静音信号、解除旧 service 的 bridge 双向引用并清空 `_avt/_rc/_cm`；`set_services()` 在 DLNA 服务重启时幂等重连，并立即以持续存活的 Player/Bridge 为真值源恢复 TransportState、位置、时长、原始 URI/元数据、Volume 与 Mute，播放中会恢复进度轮询。新 service 默认音量不会反向覆盖播放器；测试验证无需额外 `stateChanged` 即可恢复完整状态，且旧 service 不再被写入。
